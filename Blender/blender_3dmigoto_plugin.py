...
bl_info = {
    "name": "3DMigoto",
    "blender": (2, 80, 0),
    "author": "Ian Munsie (darkstarsword@gmail.com), SilentNightSound#7430 Flamindemigod",
    "location": "File > Import-Export",
    "description": "Imports meshes dumped with 3DMigoto's frame analysis and exports meshes suitable for re-injection",
    "category": "Import-Export",
    "tracker_url": "https://github.com/andradesavio9073/AGMG-Tools",
}
...
# {
#     diffuse_flat: PathBuf
#     diffuse_alpha: PathBuf
#     lightmap_flat: PathBuf
#     normalmap_flat: PathBuf
# }

def newTex(material, texture_name):
    tex = material.node_tree.nodes.new("ShaderNodeTexImage")
    bpy.data.images.load(texture_name, check_existing=True)
    tex.image = bpy.data.images[os.path.basename(texture_name)]
    tex.image.alpha_mode = "NONE"
    return tex

def newMat(name, textures):
    ...
    # material.node_tree.links.new(diffuse_flat_image.outputs[0], bsdf.inputs[0])

    #Diffuse Alpha Texture
    diffuse_alpha_image = newTex(material, textures["diffuse_alpha"])
    multiply = material.node_tree.nodes.new("ShaderNodeMix")
    multiply.blend_type = "MULTIPLY"
    multiply.clamp_factor = True
    multiply.data_type = "RGBA"
    multiply.inputs[0].default_value = 1
    material.node_tree.links.new(diffuse_flat_image.outputs[0], multiply.inputs[-1])
    ...
    #Normal Flat Texture
    if textures["normalmap_flat"]:
        normal_flat_texture = newTex(material, textures["normalmap_flat"])
        normal_map = material.node_tree.nodes.new("ShaderNodeNormalMap")
        normal_map.space = "BLENDER_WORLD"
        material.node_tree.links.new(normal_flat_texture.outputs[0], normal_map.inputs[1])
        material.node_tree.links.new(normal_map.outputs[0], bsdf.inputs[22])
...
def import_files(context: bpy.types.Context,keys, files):
    importedmeshes=[]
    for (key, file) in zip(keys, files):
        # Create a material for the mesh
        material_name = "mat_" + key
        ...
        for obj in bpy.data.objects:
            print(f"Checking {obj.name} agaist {key}")
...
def post_import_3dmigoto(operator, context, paths, load_tex=True, **kwargs):
    ...
    parse = lambda key, file_type, fallback: ([os.path.join(textures_dir ,f) for f in files if f.startswith(key) and f.endswith(file_type)] or [fallback])[0]

    for key in keys: 
        file_array.append({
                "diffuse_flat": parse(key, "DiffuseFlat.png", None),
                "diffuse_alpha": parse(key, "DiffuseAlpha.png", None),
...
def create_mod_folder(parent_folder):
    if not os.path.isdir(os.path.join(parent_folder, f"Output")):
        print(f"Creating Output")
        os.mkdir(os.path.join(parent_folder, f"Output"))
    else:
        print(f"WARNING: Everything currently in the Output folder will be overwritten - make sure any important files are backed up. Press any button to continue")
...
class Import3DMigotoFrameAnalysis(bpy.types.Operator, ImportHelper, IOOBJOrientationHelper):
    ...
    load_tex : BoolProperty(
            name="Load Load Textures from Textures Folder",
            description="Load Textures Generated from the Scaffolded Project",
            default=True,
            )
    ...
    def execute(self, context):
        ...
        try:
            ...
            post_import_3dmigoto(self, context, paths, **keywords)
        except Fatal as e:
